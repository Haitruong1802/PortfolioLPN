  ...
  return createPortal(
    <>
      ...
      <motion.div
        ...
        exit={{ opacity: 0 }}
        transition={{ duration: 0.25 }}
        className="fixed inset-y-0 left-0 right-8 z-[121] flex flex-col items-center justify-center gap-3 p-4 sm:right-0 sm:p-10"
        onClick={onClose}
        role="dialog"
        aria-modal="true"
      >
        ...
        <button
          ...
          onClick={onClose}
          aria-label="Close"
          className="absolute right-3 top-6 z-10 hidden h-11 w-11 place-items-center rounded-full border border-white/20 bg-white/10 text-white backdrop-blur hover:bg-white/20 sm:grid sm:right-8 sm:top-8"
        >
          <X className="h-5 w-5" />
        </button>
